...
void SurInterface::nbrchromatique(int chrom){
    txt->setText("Le nombre de coleurs du graphe est :" );
...
void SurInterface::generateur()
{
    ...
    form.addRow(new QLabel("Est ce que le graphe est Oriente:"));
    ...
    if (dialog.exec() == QDialog::Accepted) {
            ...
            for (int i = 0; i < nbrdesommet; i++) {
                // Genere la postion du sommet aleatoirement sur l'interface
                int a = qrand() % (480) + 420;
                int b = -(qrand() % (250)) + 320;
                QPoint pos(a,b);
                // Creation du sommet
